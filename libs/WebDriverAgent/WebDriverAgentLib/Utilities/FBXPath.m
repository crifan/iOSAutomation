                        ...
                        writer:(xmlTextWriterPtr)writer
{
  ...
  if ([root isKindOfClass:XCUIElement.class]) {
    ...
    // 20200426 CrifanLi, Note: disable following fb_waitUntilSnapshotIsStable, for it always end with (default 5 seconds) timeout
    //  disable it can speed up get page source
    // BUT: after disable, some page will error:
    // Internal error: Error Domain=com.apple.dt.xctest.automation-support.error Code=5 "Error kAXErrorServerNotFound getting snapshot for element
    // so enable again, but decrease snapshot wait stable timeout in XCUIElement+FBUtilities.m FB_ANIMATION_TIMEOUT from 5.0 to 0.5
    BOOL isContainVisible = [snapshotAttributes containsObject:FB_XCAXAIsVisibleAttributeName];
    // if ([snapshotAttributes containsObject:FB_XCAXAIsVisibleAttributeName] || 0 == snapshotAttributes.count) {
    if (isContainVisible || 0 == snapshotAttributes.count) {
