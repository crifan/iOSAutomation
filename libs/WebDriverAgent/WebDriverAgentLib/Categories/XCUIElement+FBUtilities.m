...
//static const NSTimeInterval FB_ANIMATION_TIMEOUT = 5.0;
 static const NSTimeInterval FB_ANIMATION_TIMEOUT = 0.5; // workable, not freeze
// static const NSTimeInterval FB_ANIMATION_TIMEOUT = 0.2; // will freeeze for get source of some page
//static const NSTimeInterval FB_ANIMATION_TIMEOUT = 0.3; // also freeeze
...
//  return result;
  ...
  long wait_result = dispatch_semaphore_wait(sem, absoluteTimeout);
  BOOL isWaitOk = 0 == wait_result;
  if (isWaitOk){
    [FBLogger logFmt:@"OK for application finished animations after wait %.2f seconds", timeout];
  } else {
    [FBLogger logFmt:@"The applicaion has still not finished animations after %.2f seconds timeout", timeout];
  }
  return isWaitOk;
